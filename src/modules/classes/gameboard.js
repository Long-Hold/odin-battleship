...
export class Gameboard {
  ...
  static getRandomCoordinate() {
    const randomChar = String.fromCharCode(
      Math.floor(
        Math.random() *
          (Gameboard.BOUNDS.COL.END - Gameboard.BOUNDS.COL.START + 1) +
    ...
    const randomNum = Math.floor(
      Math.random() *
        (Gameboard.BOUNDS.ROW.END - Gameboard.BOUNDS.ROW.START + 1) +
  ...
  static getAdjacentCoordinates(referenceCoordinate) {
    if (this.isOutOfBounds(referenceCoordinate)) return [];
  ...
  static getAxisDirection(startCoordinate, endCoordinate) {
    ...
    if (startCoord === endCoord)
      throw new Error(
        "A direction cannot be determined if both coordinates are the same",
      );
     ...
     */
    if (columnDiff !== 0) return this.AXIS.ROW;
    if (rowDiff !== 0) return this.AXIS.COL;
  }
  ...
  static getSiblingCoordinate(
    ...
    if (
      siblingDir !== this.AXIS.ASCENDING &&
      siblingDir !== this.AXIS.DESCENDING
    )
  ...
  static getCoordinateDirection(startCoordinate, endCoordinate) {
    ...
    if (this.isOutOfBounds(startCoord) || this.isOutOfBounds(endCoord))
      throw new Error(
        `${this.isOutOfBounds(startCoord) ? startCoord : endCoord} is not a valid coordinate`,
      );
    if (startCoord === endCoord)
  ...
  placeShip(coordinates, shipType) {
    ...
    if (normalizedCoords.length !== ship.length)
      throw new Error(
        `coordinates range (${coordinates}) is larger than ship length (${ship.length})`,
      );
  ...
  recordShipAxis(shipType, direction) {
    ...
    else this.shipAxis.set(ship, Gameboard.AXIS.COL);
   ...
   */
  allShipsSunk() {
    return [...this.#placedShips.values()].every(
