...
export class Computer extends Player {
  ...
  #getRandomAttack() {
    while (true) {
      ...
      if (this.gameBoard.guessedSpaces.has(coordinate)) continue;
  ...
  getAttack() {
    ...
    while (this.#targeting.queue.length > 0) {
      ...
      if (!this.gameBoard.guessedSpaces.has(next)) return next;
    }
  ...
  queueAdjacentAttacks(referenceCoordinate) {
    ...
    if (this.#targeting.lockedDirection) {
      ...
      if (!nextAttack) this.#reverseAndEnqueue();
      else this.#targeting.queue.push(nextAttack);
    } else if (this.#targeting.initialStrike) {
      this.#targeting.queue.length = 0;
      // The axis the ship is FACING
      this.#targeting.facingAxis = Gameboard.getAxisDirection(
        this.#targeting.initialStrike,
        referenceCoord,
      );
      // Now I can calculate whether or not this is reference coord is higher or lower than the initial strike
      this.#targeting.lockedDirection = Gameboard.getCoordinateDirection(
        this.#targeting.initialStrike,
        referenceCoord,
      );
    ...
    // Otherwise all adjacent coordinates need to be checked until another hit is found or queue is exhausted
      ...
      const adjacentCoordinates = [
        ...Gameboard.getAdjacentCoordinates(referenceCoord),
      ];
      this.#targeting.queue = adjacentCoordinates.filter(
        (coord) => this.gameBoard.guessedSpaces.has(coord) === false,
      );
   ...
   */
  handleMissedAttack() {
    if (!this.#targeting.initialStrike || !this.#targeting.lockedDirection)
      return;
