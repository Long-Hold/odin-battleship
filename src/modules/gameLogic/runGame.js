...
export async function runGame(game, coordinateRetriever) {
  ...
  while (true) {
    ...
    const [current, opponent] =
      game.getCurrentPlayer() === playerOne
    ...
    if (attackResult.isSunk) {
      const sunkShipCoordinates =
        opponent.player.gameBoard.getShipCoordinates(coord);
    ...
    if (current.player === playerTwo) {
      if (attackResult.isSunk) playerTwo.clearTargetingData();
      else if (attackResult.isHit === true)
        playerTwo.queueAdjacentAttacks(coord);
      else playerTwo.handleMissedAttack();
    }
