...
describe("class Gameboard", () => {
  ...
  describe("static methods", () => {
    describe("Gameboard.isOutOfBounds()", () => {
      test.each([[], 123, {}])(
        "returns true when passed non-string param",
        (input) => {
    ...
    describe("Gameboard.getAdjacentCoordinates()", () => {
      test.each(["A11", "K1", "", "  ", 1, [], null])(
        "returns an empty array when passed an invalid coordinate",
        (input) => {
      ...
      test.each([
        ...
        "expect $coord to have length: $length and adjacent coords of $adjCoords",
        ({ coord, length, adjCoords }) => {
    ...
    describe("Gameboard.getAxisDirection()", () => {
      ...
      test("throws Error when one or both parameters are invalid coordinates", () => {
        ...
        // Both params invalid
        expect(() =>
          Gameboard.getAxisDirection(invalidOne, invalidTwo),
        ).toThrow(Error);

        // First param invalid
        expect(() => Gameboard.getAxisDirection(invalidOne, validTwo)).toThrow(
          Error,
        );

        // Second param invalid
        expect(() => Gameboard.getAxisDirection(validOne, invalidTwo)).toThrow(
          Error,
        );

        //Neither param invalid
        expect(() =>
          Gameboard.getAxisDirection(validOne, validTwo),
        ).not.toThrow();
      });
      test.each([
        ["A1", "J10"],
      ...
      test.each([
        ...
        "does not throw when passed coordinates on the same axis",
        (start, end) => {
      ...
      test.each([
        ...
        "[$start, $end] return the axis direction: $axis",
        ({ start, end, axis }) => {
    ...
    describe("Gameboard.getSiblingCoordinate()", () => {
      test("throws Error for invalid axis", () => {
        expect(() =>
          Gameboard.getSiblingCoordinate("A1", "LETTER", "ASCENDING"),
        ).not.toThrow();
        expect(() =>
          Gameboard.getSiblingCoordinate("F8", "NUMBER", "DESCENDING"),
        ).not.toThrow();
        expect(() =>
          Gameboard.getSiblingCoordinate("J10", "ROW", "ASCENDING"),
        ).toThrow(Error);
      });
      test("throws Error for invalid direction", () => {
        ...
        expect(() =>
          Gameboard.getSiblingCoordinate("A1", "LETTER", "increase"),
        ).toThrow(Error);
      });
      describe("return values for reference coordinates facing LETTER axis", () => {
        test.each([
          {
            referenceCoord: "A1",
            referenceAxis: "LETTER",
            siblingDir: "ASCENDING",
            expected: "A2",
          },
          {
            referenceCoord: "J9",
          ...
          "returns $expected when passed $referenceCoord and $siblingDir",
          ({ referenceCoord, referenceAxis, siblingDir, expected }) => {
            expect(
              Gameboard.getSiblingCoordinate(
                referenceCoord,
                referenceAxis,
                siblingDir,
              ),
    ...
    describe("Gameboard.getCoordinateDirection()", () => {
      ...
      test("throws Error when passed duplicate coordinates", () => {
        expect(() => Gameboard.getCoordinateDirection("A1", "A1")).toThrow(
          Error,
        );
        expect(() => Gameboard.getCoordinateDirection("j10", "J10")).toThrow(
          Error,
        );
      });
    });
  ...
  describe("Gameboard.placeShip()", () => {
    test("throws TypeError if coordinates are not an array", () => {
      expect(() => gameBoard.placeShip("A1A2A3", "submarine")).toThrow(
        TypeError,
      );
    });
    test.each([
      ...
      "throws RangeError if coordinates array has invalid coordinate",
      (coordinates) => {
        expect(() => gameBoard.placeShip(coordinates, "destroyer")).toThrow(
          RangeError,
        );
      },
    );
    test.each([
      ...
      "throws Error if coordinates array length is larger than ship length",
      ({ coords, ship }) => {
    ...
    test("throws Error if a coordinate is already occupied", () => {
      ...
      expect(() =>
        gameBoard.placeShip(["A1", "A2", "A3"], "destroyer"),
      ).toThrow(Error);
      // If second coordinate is occupied
      expect(() => gameBoard.placeShip(["A1", "B1"], "patrolboat")).toThrow(
        Error,
      );
      // If third coordinate is occupied
      expect(() =>
        gameBoard.placeShip(["A1", "B1", "C1", "D1"], "battleship"),
      ).toThrow(Error);
    });
    test("throws Error if a duplicate ship is added", () => {
      expect(() =>
        gameBoard.placeShip(["A1", "B1"], "patrolboat"),
      ).not.toThrow();
      expect(() => gameBoard.placeShip(["A2", "B2"], "patrolboat")).toThrow(
        Error,
      );
    });
  ...
  describe("Gameboard.receiveAttack()", () => {
    test.each(["A0", "A11", "J0", "J11", "K4", {}, "", "  ", 123.23])(
      "throws RangeError for invalid coordinates",
      (input) => {
    ...
    test.each(["A1", "J10", "I10", "J6", "A3"])(
      "receiveAttack.isSunk returns False when a ship is present on coordinate, but not sunk",
      (coord) => {
    ...
    test.each([
      ...
      "receiveAttack.isSunk returns True when a ship is sunk",
      ({ coordsArray }) => {
  ...
  describe("Gameboard.getShipCoordinates()", () => {
    ...
    test("returns all coordinates the ship occupies", () => {
      expect(gameBoard.getShipCoordinates("A1")).toEqual(
        expect.arrayContaining(["A1", "A2"]),
      );
      expect(gameBoard.getShipCoordinates("A1")).toHaveLength(2);

      expect(gameBoard.getShipCoordinates("C1")).toEqual(
